...
class AffiliateWindowImporter < AffilinetImporter
  ...
  def check_details(node, values)
    ...
    return unless node.name == DETAIL_TAG
  ...
  def check_price(node, values)
    return unless node.name == self.class::PRICATE_TAG
  ...
  def product_remote_image(values)
    return values[IMAGES].gsub('130/130', '600/600').gsub('300/300', '600/600') if values[IMAGES].present?
