...
class Product < ActiveRecord::Base
  ...
  def similar_products
    category = categories.where(leaf: true).first
    return Product.none if category.blank?

    products = category.products.where(published: true)
    products = products.where(colorization_id: colorization_id) if colorization.present?
    products.where('products.id != ?', id).group('products.id').offset(rand(products.count)).limit(4)
  end
  ...
  def self.dedupe(affiliate_id)
    ...
    grouped.each_value do |duplicates|
      ...
      counter += 1 if duplicates.length.positive?
