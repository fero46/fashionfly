...
class Affiliate < ActiveRecord::Base
  ...
  def self.IMPORTERS
    %i[GenericImporter AffilinetImporter GaleriaImporter
  ...
  def start_import
    if replace_only_images
      ...
      products.where('id > ?',
                     start_from_id).where(published: true).order(:id).find_in_batches(batch_size: 500) do |group|
        group.each do |product|
          ...
          actual_counter += 1
          self.percent = ((actual_counter.to_f / total_counter).to_f * 100).to_i if (actual_counter % 20).zero?
          save
        end
      end
